...
export function resolveEnsureDevRequest({
  ...
  const hasExplicitPortEnd = Boolean(options.portEnd || env.EXPLORER_PORT_END);
...
function buildNoAvailablePortError({ port, portEnd, bindFailures }) {
  ...
  if (deniedFailures.length) {
    return new Error(
      `No available CAD Explorer port found in ${port}-${portEnd}; local port binding was denied (${summarizeBindFailures(deniedFailures)}). ` +
      "Rerun dev:ensure with local binding permission/escalation, or point --port at an already-running CAD Explorer server."
    );
  }

  if (bindFailures.length) {
    return new Error(
      `No available CAD Explorer port found in ${port}-${portEnd}; bind failures: ${summarizeBindFailures(bindFailures)}`
    );
  }

  return new Error(`No available CAD Explorer port found in ${port}-${portEnd}`);
}
...
export async function selectExplorerServer({
  ...
  canBind = canBindPort,
  registeredServers = readExplorerServerRegistry(),
} = {}) {
...
Options:
  ...
  --port-end <number>      Last port to probe. Defaults to 4198, then auto-expands
                           to 80 more ports unless EXPLORER_PORT_END is set.
...
export async function runEnsureDev(argv = process.argv.slice(2), {
  ...
  writeExplorerServerRegistry(serverInfo);
